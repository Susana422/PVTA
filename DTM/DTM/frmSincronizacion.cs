    ...
    public partial class frmSincronizacion : RibbonForm
    {
        ...
        public frmSincronizacion()
        {
            ...
            sucursales = (from x in Sucursal.Sucursales()
                          where x.activo == true
                          select x).ToList();
            servidor_local = Program.dtm.Conexion.DataSource;
        ...
        private async void InicializarSAP()
        {
            try
            {
                if (Program.dtm.UsuarioAutenticado.rol != 'A' && Program.dtm.UsuarioAutenticado.rol != 'X')
                {
                    solo_bajada = true;
                }
                else
                ...
                NSAP = new NSAP(Configuracion.SAP.Obtener(), solo_bajada);
                bool conexion_sap = false;
                if (!solo_bajada)
                {
                    conexion_sap = await NSAP.Conectar();
                }
                if (!conexion_sap && !solo_bajada)
                {
                    MessageBox.Show(NSAP.ObtenerUltimoError().Message.ToString().Replace("Nori", "DTM"));
                }
                if (solo_bajada || conexion_sap)
                {
                    NSAP.ReportProgress = (EventHandler<NSAP.ProgressArgs>)Delegate.Combine(NSAP.ReportProgress, new EventHandler<NSAP.ProgressArgs>(bw_ReportProgress));
                    bw.RunWorkerAsync();
                }
                else
                {
        ...
        protected void bw_ReportProgress(object sender, NSAP.ProgressArgs e)
        {
            try
            {
            ...
            try
            {
                foreach (Sucursal sucursal in sucursales)
                {
                    try
                    {
                        ...
                        goto end_IL_002e;
                    IL_0205:
                        lbLogs.Items.Add(NSAP.Sincronizar(sucursal));
                    end_IL_002e:;
                    }
                    catch
                    {
        ...
        private void frmSincronizacion_Load(object sender, EventArgs e)
        {
            try
            {
                if (Program.dtm.Configuracion.sap && MessageBox.Show("Desea iniciar la sincronización", ((Control)this).Text, MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    InicializarSAP();
                }
                timer.Enabled = true;
