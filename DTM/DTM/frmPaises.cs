...
namespace DTM
{
    public partial class frmPaises : RibbonForm
    {
        ...
        {
            ((Control)(object)lblID).Text = pais.id.ToString();
            ((Control)(object)txtCodigo).Text = pais.codigo;
            ((Control)(object)txtNombre).Text = pais.nombre;
            ((Control)(object)lblFechaActualizacion).Text = pais.fecha_actualizacion.ToShortDateString();
            if (nuevo)
            {
        ...
        private void Llenar()
        {
            pais.codigo = ((Control)(object)txtCodigo).Text;
            pais.nombre = ((Control)(object)txtNombre).Text;
        }
        ...
        private void Buscar()
        {
            try
            {
                ...
                DataTable dataTable = Utilidades.Busqueda("paises", objeto, parametros);
                if (dataTable.Rows.Count > 0)
                {
                    if (dataTable.Rows.Count == 1)
                    {
                        pais = Pais.Obtener((int)dataTable.Rows[0]["id"]);
                        Cargar();
                        return;
                    }
                    ...
                    {
                        pais = Pais.Obtener(frmResultadosBusqueda2.id);
                        Cargar();
                    }
                }
                else
        ...
        {
            if (e.KeyCode == Keys.Return && pais.id == 0)
            {
                Buscar();
            }
        }
