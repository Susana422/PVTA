...
namespace NORI
{
    public partial class btnBuscar : RibbonForm
    {
        ...
        public btnBuscar()
        {
            ...
            // Agregar filas
            dt.Rows.Add(1, "CO", "Oferta de Venta");
            dt.Rows.Add(2, "PE", "Orden de Venta");
            dt.Rows.Add(3, "FA", "Factura");
            ((RepositoryItemLookUpEditBase)cbDocumentos.Properties).DataSource = dt;
            ((RepositoryItemLookUpEditBase)cbDocumentos.Properties).ValueMember = "codigo";
            ((RepositoryItemLookUpEditBase)cbDocumentos.Properties).DisplayMember = "nombre";
        ...
        private void RegistrarEventosDeClic()
        {
            ...
            foreach (Control control in this.xtraTabPageFinanzas.Controls)
            {
                control.Click += new EventHandler(Control_Click);
            }
            foreach (Control control in this.xtraTabPageDocumentoElectronico.Controls)
            {
                control.Click += new EventHandler(Control_Click);
            }
            foreach (Control control in this.xtraTabPageAnexos.Controls)
            {
                control.Click += new EventHandler(Control_Click);
            }
        }
        ...
        private void Control_Click(object sender, EventArgs e)
        {
            ...
            {
                GridControl gridControl = clickedControl as GridControl;
                ...
                if (gridControl != null)
                {
                    ...
                    if (gridView != null)
                    {
                        // Recorrer todas las columnas del GridView
                        foreach (GridColumn column in gridView.Columns)
                        {
                            string nombreColumna = column.FieldName; // Nombre de la columna
                            string descripcionColumna = column.Caption; // Descripción (o título) de la columna

                            // Crear un nuevo objeto ParametroGrid para agregarlo a parametrizacionForm2
                            ParametroGrid nuevaFila = new ParametroGrid
                            {
                                id = 0, // O el valor que necesites
                                Nombre = nombreColumna,
                                Descripcion = descripcionColumna,
                                Oculto = false,
                                Desactivado = false
                            };

                            // Agregar la fila a parametrizacionForm2
                            parametrizacionForm2.Add(nuevaFila);
                        }
                    }
                }
            ...
            if (result == DialogResult.Yes)
            {
                foreach (DataGridViewRow dataRow in dataGridView1.Rows)
                {
                    try
                    {
                        ...
                        parametrizacionFormulario.objeto = cbDocumentos.EditValue.ToString();
                        parametrizacionFormulario.formulario = "frmDocumentos";
        ...
        {
            char.TryParse(cbRoles.EditValue.ToString(), out var result2);
            string objeto = cbDocumentos.EditValue.ToString();
            string formulario = "frmDocumentos";
            List<ParametrizacionFormulario> parametrizacionForm = ParametrizacionFormulario.ObtenerRol("frmDocumentos", objeto, result2);
