...
namespace DTM
{
    public partial class frmMonedas : RibbonForm
    {
        ...
        {
            ((Control)(object)lblID).Text = moneda.id.ToString();
            ((Control)(object)txtCodigo).Text = moneda.codigo;
            ((Control)(object)txtSimbolo).Text = moneda.simbolo.ToString();
            ((Control)(object)txtNombre).Text = moneda.nombre;
            cbActivo.Checked = moneda.activo;
            ((Control)(object)lblFechaActualizacion).Text = moneda.fecha_actualizacion.ToShortDateString();
            if (nuevo)
            {
        ...
        private void Llenar()
        {
            moneda.codigo = ((Control)(object)txtCodigo).Text;
            moneda.simbolo = char.Parse(((Control)(object)txtSimbolo).Text);
            moneda.nombre = ((Control)(object)txtNombre).Text;
        ...
        private void bbiPrimero_ItemClick(object sender, ItemClickEventArgs e)
        {
            try
            {
                moneda = (from x in Moneda.Monedas()
                          orderby x.id
                          select x).First();
            }
            catch (Exception ex)
        ...
        private void Buscar()
        {
            try
            {
                ...
                {
                    fields = "id, codigo, simbolo, nombre, activo",
                    like = true
                };
                object objeto = new
                {
                    codigo = ((Control)(object)txtCodigo).Text,
                    simbolo = ((Control)(object)txtSimbolo).Text,
                    nombre = ((Control)(object)txtNombre).Text
                };
                DataTable dataTable = Utilidades.Busqueda("monedas", objeto, parametros);
                if (dataTable.Rows.Count > 0)
                {
                    if (dataTable.Rows.Count == 1)
                    {
                        moneda = Moneda.Obtener((int)dataTable.Rows[0]["id"]);
                        Cargar();
                        return;
                    }
                    ...
                    {
                        moneda = Moneda.Obtener(frmResultadosBusqueda2.id);
                        Cargar();
                    }
                }
                else
        ...
        {
            if (e.KeyCode == Keys.Return && moneda.id == 0)
            {
                Buscar();
            }
        }
