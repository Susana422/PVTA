...
namespace DTM
{
    public partial class frmZonas : RibbonForm
    {
        ...
        {
            ((Control)(object)lblID).Text = zona.id.ToString();
            ((Control)(object)txtCodigo).Text = zona.codigo;
            ((Control)(object)txtNombre).Text = zona.nombre;
            ((Control)(object)txtDescripcion).Text = zona.descripcion;
            cbActivo.Checked = zona.activo;
            ((Control)(object)lblFechaActualizacion).Text = zona.fecha_actualizacion.ToShortDateString();
            if (nuevo)
            {
        ...
        private void Llenar()
        {
            zona.codigo = ((Control)(object)txtCodigo).Text;
            zona.nombre = ((Control)(object)txtNombre).Text;
            zona.descripcion = ((Control)(object)txtDescripcion).Text;
        ...
        private void Buscar()
        {
            try
            {
                ...
                DataTable dataTable = Utilidades.Busqueda("zonas", objeto, parametros);
                if (dataTable.Rows.Count > 0)
                {
                    if (dataTable.Rows.Count == 1)
                    {
                        zona = Zona.Obtener((int)dataTable.Rows[0]["id"]);
                        Cargar();
                        return;
                    }
                    ...
                    {
                        zona = Zona.Obtener(frmResultadosBusqueda2.id);
                        Cargar();
                    }
                }
                else
        ...
        {
            if (e.KeyCode == Keys.Return && zona.id == 0)
            {
                Buscar();
            }
        }
