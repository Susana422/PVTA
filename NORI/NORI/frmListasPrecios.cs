...
namespace NORI
{
    public partial class frmListasPrecios : RibbonForm
    {
        private ListaPrecio lista_precio = new ListaPrecio();
        ...
        {
            ((Control)(object)lblID).Text = lista_precio.id.ToString();
            ((Control)(object)txtCodigo).Text = lista_precio.codigo.ToString();
            ((Control)(object)txtNombre).Text = lista_precio.nombre;
            cbActivo.Checked = lista_precio.activo;
            ((Control)(object)lblFechaActualizacion).Text = lista_precio.fecha_actualizacion.ToShortDateString();
            if (nuevo)
            {
        ...
        private void Llenar()
        {
            lista_precio.codigo = short.Parse(((Control)(object)txtCodigo).Text);
            lista_precio.nombre = ((Control)(object)txtNombre).Text;
        ...
        private void bbiPrimero_ItemClick(object sender, ItemClickEventArgs e)
        {
            try
            {
                lista_precio = (from x in ListaPrecio.ListasPrecios()
                                orderby x.id
                                select x).First();
            }
            catch (Exception ex)
        ...
        private void Buscar()
        {
            try
            {
                ...
                DataTable dataTable = Utilidades.Busqueda("listas_precios", objeto, parametros);
                if (dataTable.Rows.Count > 0)
                {
                    if (dataTable.Rows.Count == 1)
                    {
                        lista_precio = ListaPrecio.Obtener((int)dataTable.Rows[0]["id"]);
                        Cargar();
                        return;
                    }
                    ...
                    {
                        lista_precio = ListaPrecio.Obtener(frmResultadosBusqueda2.id);
                        Cargar();
                    }
                }
                else
        ...
        {
            if (e.KeyCode == Keys.Return && lista_precio.id == 0)
            {
                Buscar();
            }
        }
        ...
        {
            frmUsuariosListasPrecios frmUsuariosListasPrecios2 = new frmUsuariosListasPrecios();
            ((Control)(object)frmUsuariosListasPrecios2).Show();
        }
