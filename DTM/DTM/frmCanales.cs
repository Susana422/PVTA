...
namespace DTM
{
    public partial class frmCanales : RibbonForm
    {
        ...
        {
            ((Control)(object)lblID).Text = canal.id.ToString();
            ((Control)(object)txtCodigo).Text = canal.codigo;
            ((Control)(object)txtNombre).Text = canal.nombre;
            cbActivo.Checked = canal.activo;
            ((Control)(object)lblFechaActualizacion).Text = canal.fecha_actualizacion.ToShortDateString();
            if (nuevo)
            {
        ...
        private void Llenar()
        {
            canal.codigo = ((Control)(object)txtCodigo).Text;
            canal.nombre = ((Control)(object)txtNombre).Text;
        ...
        private void Buscar()
        {
            try
            {
                ...
                DataTable dataTable = Utilidades.Busqueda("canales", objeto, parametros);
                if (dataTable.Rows.Count > 0)
                {
                    if (dataTable.Rows.Count == 1)
                    {
                        canal = Canal.Obtener((int)dataTable.Rows[0]["id"]);
                        Cargar();
                        return;
                    }
                    ...
                    {
                        canal = Canal.Obtener(frmResultadosBusqueda2.id);
                        Cargar();
                    }
                }
                else
        ...
        {
            if (e.KeyCode == Keys.Return && canal.id == 0)
            {
                Buscar();
            }
        }
