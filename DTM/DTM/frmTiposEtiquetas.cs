...
namespace DTM
{
    public partial class frmTiposEtiquetas : RibbonForm
    {
        private Etiqueta.Tipo tipo = new Etiqueta.Tipo();
        ...
        private LabelControl lblFormato;

        private TextEdit txtFormato;
        ...
        private void Cargar(bool nuevo = false, bool busqueda = false)
        {
            ...
            ((Control)(object)txtFormato).Text = tipo.formato;
            ...
            if (nuevo)
            {
                ...
                ((Control)(object)txtFormato).Focus();
            }
            else if (busqueda)
            {
        ...
        private void Llenar()
        {
            ...
            tipo.formato = ((Control)(object)txtFormato).Text;
        ...
        private void bbiPrimero_ItemClick(object sender, ItemClickEventArgs e)
        {
            try
            {
                tipo = (from x in Etiqueta.Tipo.Tipos()
                        orderby x.id
                        select x).First();
            }
            catch (Exception ex)
        ...
        private void Buscar()
        {
            try
            {
                ...
                DataTable dataTable = Utilidades.Busqueda("tipos", objeto, parametros);
                if (dataTable.Rows.Count > 0)
                {
                    if (dataTable.Rows.Count == 1)
                    {
                        tipo = Etiqueta.Tipo.Obtener((int)dataTable.Rows[0]["id"]);
                        Cargar();
                        return;
                    }
                    ...
                    {
                        tipo = Etiqueta.Tipo.Obtener(frmResultadosBusqueda2.id);
                        Cargar();
                    }
                }
                else
        ...
        private void btnInforme_Click(object sender, EventArgs e)
        {
            ...
            {
                ((Control)(object)txtFormato).Text = openFileDialog.SafeFileName;
            }
        }

        private void btnEditarInforme_Click(object sender, EventArgs e)
        {
            if (((Control)(object)txtFormato).Text.Length > 0)
            {
                frmDiseñadorInformes frmDiseñadorInformes2 = new frmDiseñadorInformes();
                frmDiseñadorInformes2.AbrirInforme(((Control)(object)txtFormato).Text);
