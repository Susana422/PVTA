...
namespace DTM
{
    public partial class frmChoferes : RibbonForm
    {
        ...
        private TextEdit txtLicencia;

        private LabelControl lblLicencia;

        private TextEdit txtRFC;

        private LabelControl lblRFC;

        private TextEdit txtTipoFigura;

        private LabelControl lblTipoFigura;
        ...
        {
            ((Control)(object)lblID).Text = chofer.id.ToString();
            ((Control)(object)txtCodigo).Text = chofer.codigo;
            ((Control)(object)txtNombre).Text = chofer.nombre;
            ((Control)(object)txtRFC).Text = chofer.rfc;
            ((Control)(object)txtLicencia).Text = chofer.licencia;
            ((Control)(object)txtTipoFigura).Text = chofer.tipo_figura;
            cbActivo.Checked = chofer.activo;
            ((Control)(object)lblFechaActualizacion).Text = chofer.fecha_actualizacion.ToShortDateString();
            if (nuevo)
            {
        ...
        private void Llenar()
        {
            chofer.codigo = ((Control)(object)txtCodigo).Text;
            chofer.nombre = ((Control)(object)txtNombre).Text;
            chofer.rfc = ((Control)(object)txtRFC).Text;
            chofer.licencia = ((Control)(object)txtLicencia).Text;
            chofer.tipo_figura = ((Control)(object)txtTipoFigura).Text;
        ...
        private void bbiPrimero_ItemClick(object sender, ItemClickEventArgs e)
        {
            try
            {
                chofer = (from x in Chofer.Choferes()
                          orderby x.id
                          select x).First();
            }
            catch (Exception ex)
        ...
        private void Buscar()
        {
            try
            {
                ...
                DataTable dataTable = Utilidades.Busqueda("choferes", objeto, parametros);
                if (dataTable.Rows.Count > 0)
                {
                    if (dataTable.Rows.Count == 1)
                    {
                        chofer = Chofer.Obtener((int)dataTable.Rows[0]["id"]);
                        Cargar();
                        return;
                    }
                    ...
                    {
                        chofer = Chofer.Obtener(frmResultadosBusqueda2.id);
                        Cargar();
                    }
                }
                else
        ...
        {
            if (e.KeyCode == Keys.Return && chofer.id == 0)
            {
                Buscar();
            }
        }
