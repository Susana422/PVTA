...
namespace NORI
{
    public partial class frmCausalidades : RibbonForm
    {
        private Causalidad causalidad = new Causalidad();
        ...
        {
            ((Control)(object)lblID).Text = causalidad.id.ToString();
            ((Control)(object)txtCodigo).Text = causalidad.codigo;
            ((Control)(object)txtNombre).Text = causalidad.nombre;
            cbActivo.Checked = causalidad.activo;
            ((Control)(object)lblFechaActualizacion).Text = causalidad.fecha_actualizacion.ToShortDateString();
            if (nuevo)
            {
        ...
        private void Llenar()
        {
            causalidad.codigo = ((Control)(object)txtCodigo).Text;
            causalidad.nombre = ((Control)(object)txtNombre).Text;
        ...
        private void bbiPrimero_ItemClick(object sender, ItemClickEventArgs e)
        {
            try
            {
                causalidad = (from x in Causalidad.Causalidades()
                              orderby x.id
                              select x).First();
            }
            catch (Exception ex)
        ...
        private void Buscar()
        {
            try
            {
                ...
                DataTable dataTable = Utilidades.Busqueda("causalidades", objeto, parametros);
                if (dataTable.Rows.Count > 0)
                {
                    if (dataTable.Rows.Count == 1)
                    {
                        causalidad = Causalidad.Obtener((int)dataTable.Rows[0]["id"]);
                        Cargar();
                        return;
                    }
                    ...
                    {
                        causalidad = Causalidad.Obtener(frmResultadosBusqueda2.id);
                        Cargar();
                    }
                }
                else
        ...
        {
            if (e.KeyCode == Keys.Return && causalidad.id == 0)
            {
                Buscar();
            }
        }
