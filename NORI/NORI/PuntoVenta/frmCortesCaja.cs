    ...
    public partial class frmCortesCaja : RibbonForm
    {
        ...
        private void CargarFlujo()
        {
            try
            {
                ...
                string query = (cbFlujo.Checked ? string.Format("select flujo.id ID, conceptos_flujo.nombre Concepto, flujo.codigo Código, estaciones.nombre Estación, tipos_metodos_pago.nombre 'Método de pago', flujo.tipo_cambio 'Tipo de cambio', flujo.importe Importe, flujo.referencia Referencia, flujo.comentario Comentario, autorizaciones.comentario 'Comentario autorización', usuarios.usuario, flujo.fecha_creacion 'Fecha creación', flujo.fecha_actualizacion 'Fecha actualización' from flujo left join tipos_metodos_pago on flujo.tipo_metodo_pago_id = tipos_metodos_pago.id inner join estaciones on flujo.estacion_id = estaciones.id left join autorizaciones on flujo.autorizacion_id = autorizaciones.id inner join conceptos_flujo on flujo.codigo = conceptos_flujo.codigo left join usuarios on flujo.usuario_creacion_id = usuarios.id WHERE flujo.fecha_creacion BETWEEN '{0}' AND '{1}' {2}", ((DateTime)((BaseEdit)deDesde).EditValue).ToString("yyyyMMdd"), ((DateTime)((BaseEdit)deHasta).EditValue).ToString("yyyyMMdd"), arg) : string.Format("select flujo.id ID, estaciones.nombre Estación, flujo.tipo_cambio 'Tipo de cambio', flujo.importe Importe, flujo.comentario Comentario, autorizaciones.comentario 'Comentario autorización', usuarios.usuario, flujo.fecha_creacion 'Fecha creación', flujo.fecha_actualizacion 'Fecha actualización' from flujo inner join estaciones on flujo.estacion_id = estaciones.id inner join autorizaciones on flujo.autorizacion_id = autorizaciones.id left join usuarios on flujo.usuario_creacion_id = usuarios.id WHERE flujo.codigo = 'RECCA' AND flujo.fecha_creacion BETWEEN '{0}' AND '{1}' {2}", ((DateTime)((BaseEdit)deDesde).EditValue).ToString("yyyyMMdd"), ((DateTime)((BaseEdit)deHasta).EditValue).ToString("yyyyMMdd"), arg));
